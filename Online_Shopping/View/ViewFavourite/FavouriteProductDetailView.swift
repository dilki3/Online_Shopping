//
//  FavouriteProductDetailView.swift
//  Online_Shopping
//
...
struct FavouriteProductDetailView: View {
    @Environment(\.presentationMode) var mode : Binding<PresentationMode>
    @StateObject var detailVM: FavoriteDetailViewModel = FavoriteDetailViewModel(prodfObj: ProductModel(dict: [:]) )
        ...
        ZStack{
            ...
            ScrollView{
                ZStack{
                    ...
                    WebImage(url: URL(string: detailVM.fObj.image))
                        .resizable()
                        .indicator(.activity)
                ...
                VStack{
                    ...
                    Text("\(detailVM.fObj.brandName),Brand")
                    ...
                    HStack{
                        ...
                        Spacer()
                        Text("$\((detailVM.fObj.offerPrice ?? detailVM.fObj.price) * Double(detailVM.qty), specifier: "%.2f")")
                    ...
                    if(detailVM.isShowColors) {
                        ScrollView(.horizontal, showsIndicators: false) {
                                           HStack(spacing: 10) {
                                              ...
                                              .frame(width: 30, height: 30)
                                               Circle()
                                                   .fill(Color(detailVM.fObj.color))
                                              .frame(width: 30, height: 30)
                ...
                RoundButton(title: "Add to Cart"){
                    CartViewModel.serviceCallAddToCart(prodId: detailVM.fObj.prodId, qty: detailVM.qty) { isDone, msg  in
...
#Preview {
    FavouriteProductDetailView(detailVM: FavoriteDetailViewModel(prodfObj: ProductModel(dict: [
